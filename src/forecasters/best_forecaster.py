...
class BestForecaster:
    ...
    def load_and_preprocess_data(self, sales_path, prices_path, dates_path,
        ...
        Выполняет объединение данных, фильтрацию по магазину, обработку
        пропусков (интерполяция, ffill/bfill), агрегацию по дням и
        создание базовых календарных признаков. Сбрасывает состояние
        предыдущих запусков (модели, прогнозы и т.д.).
    ...
    def predict_all_items(self, target_col='cnt', items_limit=None, horizons=None):
        """
        Запускает цикл прогнозирования для всех (или ограниченного числа)
        валидных товаров магазина.
