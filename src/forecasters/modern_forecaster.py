...
class CatBoostForecaster:
    ...
    Атрибуты:
        ...
        items_data (dict): Словарь с предобработанными дневными данными
                           для каждого товара {item_id: pd.DataFrame}.
        data (pd.DataFrame): Предобработанные агрегированные дневные данные
                             по всему магазину.
        _full_dates_data (pd.DataFrame): Исходные данные календаря.
        data_split (bool): Флаг, указывающий, разделены ли данные на train/test.
        split_results (dict): Словарь с результатами разделения данных
                              для каждого товара {item_id: (train_df, test_df)}.
        ...
        models (dict): Словарь с обученными моделями CatBoost
                       {ключ_модели: модель}.
        forecasts (dict): Словарь с сгенерированными прогнозами
                          {ключ_прогноза: pd.DataFrame}.
        metrics (dict): Словарь с метриками оценки
                        {(ключ_прогноза, горизонт): dict_с_метриками}.
    """
    ...
    def add_time_features(self, data, item_id=None):
        ...
        Args:
            data (pd.DataFrame): Входной DataFrame с DatetimeIndex и столбцами
                                 целевой переменной ('cnt') и 'sell_price'.
    ...
    def prepare_features_targets(self, target_col='cnt', item_id=None):
        ...
        Выбирает источник данных (по товару или агрегированный) на основе
        `item_id`. Определяет столбцы признаков и категориальные признаки.
        ...
        Raises:
            ValueError: Если данные не разделены или данные для `item_id`
                        не найдены в результатах разделения.
        """
    ...
    def evaluate(self, target_col='cnt', horizon='week', item_id=None):
        ...
        Получает прогноз для указанного горизонта и сравнивает его
        с соответствующей частью тестовой выборки.
    ...
    def visualize_forecast(self, target_col='cnt', horizon='week', item_id=None):
        """
        Строит график прогноза по сравнению с фактическими тестовыми данными
        и недавними обучающими данными.
