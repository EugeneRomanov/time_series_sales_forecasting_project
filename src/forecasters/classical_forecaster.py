...
class ClassicalForecaster:
    """
    Класс для прогнозирования временных рядов с использованием
    ...
    Атрибуты:
        ...
        store_id (str | None): Идентификатор магазина, для которого
                                выполнялся анализ.
        items_data (dict): Словарь, где ключи - item_id, значения -
                           DataFrame с данными для этого товара.
    ...
    def load_and_preprocess_data(self, sales_path, prices_path, dates_path,
                                 store_id='STORE_2'):
        """
        Загружает и предобрабатывает данные, фильтруя по магазину и
        организуя по товарам. Использует оригинальную логику интерполяции
        пользователя (с заменой 0 на NaN).
        ...
        Args:
            ...
            store_id (str, optional): Идентификатор магазина для анализа.
                                       По умолчанию 'STORE_2'.

        Returns:
            dict: Словарь, где ключи - item_id, а значения - pd.DataFrame
                  с обработанными временными рядами для каждого товара.
        ...
        price_means = merged_data.groupby('item_id')['sell_price'] \
                                 .transform('mean')
        merged_data['sell_price'] = merged_data['sell_price'].fillna(price_means)
        merged_data['sell_price'] = merged_data['sell_price'] \
                                           .fillna(method='bfill') \
                                           .fillna(method='ffill')
        ...
        store_daily_data['revenue'] = store_daily_data['revenue'].replace(0, np.nan) \
                                                                 .interpolate(method='linear')

        for col in agg_dict_store:
             if col not in ['cnt', 'revenue']:
                 store_daily_data[col] = store_daily_data[col].fillna(method='ffill') \
                                                              .fillna(method='bfill')
        ...
        store_daily_data['cnt'] = store_daily_data['cnt'].fillna(method='bfill') \
                                                         .fillna(method='ffill')
        store_daily_data['revenue'] = store_daily_data['revenue'].fillna(method='bfill') \
                                                                 .fillna(method='ffill')
    ...
    def train_test_split(self, test_size=0.2):
        """
        Разделяет данные на обучающую и тестовую выборки для всех товаров
        и для агрегированных данных магазина.

        Args:
            test_size (float, optional): Доля тестовой выборки (от 0 до 1).
                                         По умолчанию 0.2.

        Returns:
            dict: Словарь, где ключи - item_id, а значения - кортежи
                  (train_data, test_data) для каждого товара.

        Raises:
            ValueError: Если данные по товарам (items_data) не загружены или
                        агрегированные данные (self.data) отсутствуют/некорректны.
        """
    ...
    def train_arima_model(self, target_col, order=(2, 1, 2), item_id=None):
        ...
        Args:
            ...
            order (tuple, optional): Параметры (p, d, q) для ARIMA.
                                     По умолчанию (2, 1, 2).
            item_id (str | None, optional): Идентификатор товара. Если None,
                                            обучается модель для всего магазина.
                                            По умолчанию None.
        ...
        Raises:
            ValueError: Если данные для товара не найдены или обучающие данные
    ...
    def train_sarima_model(self, target_col, order=(1, 1, 1),
        ...
        Args:
            target_col (str): Столбец для прогнозирования.
            order (tuple, optional): Параметры (p, d, q) для ARIMA части.
                                     По умолчанию (1, 1, 1).
            seasonal_order (tuple, optional): Сезонные параметры (P, D, Q, s).
                                              По умолчанию (1, 1, 1, 7).
            item_id (str | None, optional): Идентификатор товара. Если None,
                                            обучается модель для всего магазина.
                                            По умолчанию None.
    ...
    def train_ets_model(self, target_col, seasonal_periods=7, trend='add',
        ...
        Args:
            ...
            trend (str | None, optional): Тип тренда ('add', 'mul', None).
                                          По умолчанию 'add'.
            seasonal (str | None, optional): Тип сезонности ('add', 'mul', None).
                                             По умолчанию 'add'.
            damped (bool, optional): Использовать ли затухающий тренд
                                     (damped_trend в statsmodels).
                                     По умолчанию True.
            item_id (str | None, optional): Идентификатор товара. Если None,
                                            обучается модель для всего магазина.
                                            По умолчанию None.
    ...
    def forecast(self, model_name, steps):
        ...
        Raises:
            ValueError: Если модель с таким именем не найдена, steps некорректно,
                        или не удалось определить последнюю дату обучения.
    ...
    def evaluate(self, model_name, horizon='week'):
        ...
        Args:
            ...
            horizon (str, optional): Горизонт оценки ('week', 'month', 'quarter').
                                     По умолчанию 'week'.
        ...
        Raises:
            ValueError: Если прогноз для модели не найден, горизонт некорректен,
                        или не удалось найти/выровнять тестовые данные.
    ...
    def visualize_forecast(self, model_name, horizon='week'):
        ...
        Args:
            model_name (str): Название модели (ключ в self.forecasts).
            horizon (str, optional): Горизонт визуализации ('week', 'month',
                                     'quarter'). По умолчанию 'week'.

        Returns:
            tuple: Кортеж (train_data, test_data, forecast)
                   с данными, использованными для графика (как в оригинале).

        Raises:
            ValueError: Если прогноз для модели не найден, горизонт некорректен,
                        или не удалось получить данные для визуализации.
        """
        if model_name not in self.forecasts:
            raise ValueError(f"Прогноз для модели {model_name} не найден")
        if model_name not in self.models:
    ...
    def find_best_classical_model(self, target_col, horizon='week',
        ...
        """
        Находит лучшую классическую модель (ARIMA, SARIMA, ETS) для
        прогнозирования на заданном горизонте, сравнивая их по MAE.

        Args:
            target_col (str): Целевой столбец для прогнозирования.
            horizon (str, optional): Горизонт прогнозирования ('week', 'month',
                                     'quarter'). По умолчанию 'week'.
            verbose (bool, optional): Если True, выводит подробную информацию
                                      о процессе тестирования моделей.
                                      По умолчанию False.
            item_id (str | None, optional): Идентификатор товара. Если None,
                                            ищет лучшую модель для всего магазина.
                                            По умолчанию None.
        ...
        Raises:
            ValueError: Если не удалось успешно обучить, спрогнозировать и
                        оценить ни одну модель из списка `models_to_test`.
        """
    ...
    def predict_with_best_model(self, target_col, horizon='week', steps=None,
        ...
        Args:
            ...
            horizon (str, optional): Горизонт для поиска лучшей модели
                                     ('week', 'month', 'quarter').
                                     По умолчанию 'week'.
            steps (int | None, optional): Количество шагов для прогноза.
                                          ...
                                          По умолчанию None.
            item_id (str | None, optional): Идентификатор товара. Если None,
                                            используется модель для всего магазина.
                                            По умолчанию None.
        ...
        Raises:
            ValueError: Если не удалось найти лучшую модель или сгенерировать
                        прогноз.
        """
    ...
    def find_best_models_for_all_items(self, target_col='cnt', horizon='week',
        ...
        Args:
            ...
            top_n (int, optional): Количество лучших товаров для визуализации.
                                   По умолчанию 5.
            verbose (bool, optional): Подробный вывод при поиске моделей для
                                      каждого товара. По умолчанию False.
